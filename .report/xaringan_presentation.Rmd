...
phi <- matrix(c(1,0.5, 0.5, 1), ncol = 2)
...
- total number of data: 24 * 100 = 2400
...
sim_result %>% mutate_at(vars(est, pvalue), round, 3)
```
]
]
]
...
- `#'`
- Rd 생성 in man folder
- 이게 나중에 패키지 설명으로 자동으로 렌더링 됨.
- CRAN 에 올리려면 vignettes(패키지 설명서)를 R 규정에 맞게 써야 함
